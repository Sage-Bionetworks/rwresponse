...
#' @param tol_low Tolerance surrounding dob_event_min.  The default of -0.5 will include events on dob_event_min.
#' @param tol_high Tolerance surrounding dob_event_max.  The default of 0.5 will include events on dob_event_max.
#' @export
filter_events <- function(
  ...
  if (
    !all(
      c('record_id', 'dob_event_min', 'dob_event_max') %in% colnames(df_time)
    )
  ) {
    cli::cli_abort(
      "'df_time' must have columns record_id, dob_event_min, dob_event_max"
    )
  }
